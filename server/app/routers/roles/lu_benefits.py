...
def list_benefits(*, session: Session = Depends(get_session), active_only: bool = Query(True)) -> list[StandardLookupBase]:
    return get_entity_or_404(session = session, model = rolesLuBenefit, IsActive = active_only)
...
def get_benefit(benefit_id: int, session: Session = Depends(get_session)) -> StandardLookupBase:
    return get_entity_or_404(session = session, model = rolesLuBenefit, entity_id = benefit_id)

@router.get(conf_pathname()+"/v1/roles/lookup/benefits/by-name/{benefit_name}", response_model=list[StandardLookupBase])
def get_benefit(benefit_name: str, session: Session = Depends(get_session), active_only: bool = Query(True)) -> list[StandardLookupBase]:
    return get_entity_or_404(session = session, model = rolesLuBenefit, entity_name = benefit_name, IsActive = active_only)
...
def create_or_update_benefit(payload: StandardLookupBase, session: Session = Depends(get_session)) -> StandardLookupBase:
    return upsert_entity(session = session, model = rolesLuBenefit, payload = payload)
...
def delete_benefit(benefit_id: int, session: Session = Depends(get_session)) -> StandardLookupBase:
    return soft_delete_entity(session = session, model = rolesLuBenefit, entity_id = benefit_id)
