...
def test_locations_crud_flow() -> None:
    ...
    list_response = client.get("/api/v1/repository/locations")
    assert list_response.status_code == 200
    assert any(item["Id"] == location_id for item in list_response.json())

    get_response = client.get(f"/api/v1/repository/locations/{location_id}")
    assert get_response.status_code == 200
    assert get_response.json()["Id"] == location_id

    delete_response = client.delete(f"/api/v1/repository/locations/{location_id}")
